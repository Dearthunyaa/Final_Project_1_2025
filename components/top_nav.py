...
class TopNavPanel(ctk.CTkFrame):
    ...
    def _create_stat_header(self, title, val):
        ...
        ctk.CTkLabel(f, text=title, font=("Arial", 10), text_color=COLOR_TEXT_SUB).pack(anchor="w")
        lbl = ctk.CTkLabel(f, text=val, font=("Arial", 10, "bold"), text_color=COLOR_TEXT_MAIN)
        lbl.pack(anchor="w")
        return lbl

    def _create_stat_label(self, text, color, size):
        lbl = ctk.CTkLabel(self, text=text, font=("Roboto", size, "bold"), text_color=color)
    ...
    def update_logo(self, symbol):
        try:
            ...
            if os.path.exists(image_path):
                ...
                self.logo_image = ctk.CTkImage(light_image=pil_image, dark_image=pil_image, size=(25, 25))
                self.logo_label.configure(image=self.logo_image, text="")
            else:
                self.logo_label.configure(image=None, text=symbol, font=("Arial", 12, "bold"))
    ...
    def update_data(self, ticker_data):
        ...
        self.lbl_vol.configure(text=f"{float(ticker_data['quoteVolume'])/1000000:.2f}M")
