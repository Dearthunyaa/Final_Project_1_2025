...
class RightSidebar(ctk.CTkFrame):
    ...
    def _create_orderbook(self):
        self.ob_frame = ctk.CTkFrame(self, fg_color=COLOR_BG_PANEL, corner_radius=0, border_width=1, border_color=COLOR_BORDER)
        self.ob_frame.grid(row=0, column=0, sticky="nsew", pady=(0, 5))
        
        ctk.CTkLabel(self.ob_frame, text="OrderBook", font=FONT_BOLD, text_color=COLOR_TEXT_MAIN).pack(pady=5, anchor="w", padx=SIDE_PAD)
        ...
        for _ in range(ROW_LIMIT):
            c = ctk.CTkCanvas(self.ob_container, height=20, bg=COLOR_BG_PANEL, highlightthickness=0)
            c.pack(fill="x", padx=5, pady=1)
            self.ob_rows.append(c)

    def _create_trades(self):
        self.tr_frame = ctk.CTkFrame(self, fg_color=COLOR_BG_PANEL, corner_radius=0, border_width=1, border_color=COLOR_BORDER)
        self.tr_frame.grid(row=1, column=0, sticky="nsew", pady=(0, 0))
        
        ctk.CTkLabel(self.tr_frame, text="Recent Trades", font=FONT_BOLD, text_color=COLOR_TEXT_MAIN).pack(pady=5, anchor="w", padx=SIDE_PAD)
        
        self.tr_container = ctk.CTkFrame(self.tr_frame, fg_color="transparent")
        self.tr_container.pack(fill="both", expand=True)
        self.tr_labels = []
        for _ in range(ROW_LIMIT):
            l = ctk.CTkLabel(self.tr_container, text="", font=FONT_UNIFIED, anchor="w")
    ...
    def update_orderbook(self, depth_data):
        if not depth_data or not self.ob_frame.winfo_viewable(): return
    ...
    def _draw_bar(self, canvas, p, q, max_v, is_bid):
        ...
        canvas.create_text(5, h/2, anchor="w", text=f"{p:,.2f}", fill=color_fg, font=("Consolas", 10, "bold"))
        canvas.create_text(w-5, h/2, anchor="e", text=f"{q:.4f}", fill=COLOR_TEXT_MAIN, font=("Consolas", 10))

    def update_trades(self, trades_list):
        if not trades_list or not self.tr_frame.winfo_viewable(): return
