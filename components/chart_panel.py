...
class ChartPanel(ctk.CTkFrame):
    ...
    def _create_toolbar(self):
        toolbar = ctk.CTkFrame(self, height=35, fg_color=COLOR_BG_PANEL, corner_radius=0, border_width=1, border_color=COLOR_BORDER)
        toolbar.grid(row=0, column=0, sticky="ew", pady=(0, 5))
        
        ctk.CTkFrame(toolbar, width=17, height=20, fg_color="transparent").pack(side="left") 

        tfs = ["1m", "15m", "1H", "4H", "1D"]
        for tf in tfs:
            btn = ctk.CTkButton(
                toolbar, text=tf, width=30, height=20,
                fg_color=COLOR_ACTIVE if tf.lower() == self.current_interval.lower() else COLOR_BTN_DEFAULT,
                font=("Arial", 10, "bold"),
        ...
        ctk.CTkFrame(toolbar, width=15, height=20, fg_color="transparent").pack(side="right")
    ...
    def _create_toggle_btn(self, parent, text, key):
        ...
        def on_click():
            ...
            btn.configure(fg_color=COLOR_ACTIVE if is_on else COLOR_BTN_DEFAULT)
    ...
    def _create_chart_canvas(self):
        self.chart_frame = ctk.CTkFrame(self, fg_color=COLOR_BG_MAIN, border_width=1, border_color=COLOR_BORDER)
        ...
        self.fig.subplots_adjust(left=0.02, right=0.88, top=0.98, bottom=0.04, hspace=0.05)
    ...
    def update_chart(self, df):
        ...
        try:
            ...
            self.ax1.yaxis.set_major_locator(MaxNLocator(nbins=6, prune='lower')) 
            self.ax2.yaxis.set_major_locator(MaxNLocator(nbins=3, prune='upper')) 
            ...
            self.ax1.text(0.01, 0.94, f"MA(20): {v_ma20:,.2f}", transform=self.ax1.transAxes, color='#00e5ff', fontsize=9, fontweight='bold', ha='left')
            self.ax1.text(0.01, 0.89, f"MA(50): {v_ma50:,.2f}", transform=self.ax1.transAxes, color='#ff9900', fontsize=9, fontweight='bold', ha='left')
            ...
            self.ax1.axhline(last_price, color=color_tag, linestyle='--', linewidth=0.8, alpha=0.8)
            self.ax1.text(
            ...
            self.ax2.tick_params(axis='x', colors='white', labelsize=8, labelbottom=True, pad=5)
            self.ax2.grid(False) 
            for collection in self.ax2.collections: collection.set_linewidth(0) 

            self.chart_canvas.draw()
        except Exception as e: print(f"Chart Drawing Error: {e}")
